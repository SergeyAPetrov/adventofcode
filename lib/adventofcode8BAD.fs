module adventofcode8BAD

open System.Collections.Generic

let getComparator (condition:string) = 
    ...
    | "!=" -> (<>)
    | "==" -> (=)
    | _ -> failwith "unknown comparator"

let evalCondition left condition right = 
    let func = getComparator condition
    func left right

let getOperator (op:string) = 
    match op with 
    | "inc" -> (+)
    | "dec" -> (-)
    | _ -> failwith "unknown operator"

let evalOperation left op right = 
    let func = getOperator op
    func left right

let getRegisterValue (state:IDictionary<string, int>) (name:string) = 
    if state.ContainsKey(name) then
        state.[name]
    else
        state.[name] <- 0
        0

let processRow (state:IDictionary<string, int>) (row:string) = 
    let parts = row.Split(' ')
    let registerToCompare = getRegisterValue state parts.[4]
    let valueToCompare = parts.[6] |> System.Int32.Parse
    if evalCondition registerToCompare parts.[5] valueToCompare then
        let registerToModify = getRegisterValue state parts.[0]
        let valueToModify = parts.[2] |> System.Int32.Parse
        state.[parts.[0]] <- evalOperation registerToModify parts.[1] valueToModify
    let max = Seq.maxBy (fun (x:KeyValuePair<string, int>) -> x.Value) state
    max.Value

let solve (input:string) =
    let state = new System.Collections.Generic.Dictionary<string,int>()
    input.Split('\n')
    |> Array.map (fun x -> processRow state x)
    |> Array.max
    //let max = Seq.maxBy (fun (x:KeyValuePair<string, int>) -> x.Value) state
    //max.Value
